...
- **其他更改**
  ...
  - &nbsp;<Tag>调整</Tag> 审计日志日志 `LoggingMonitor` 返回值泛型字符串显示格式 <sup>4.8.7.1</sup> <sup>⏱️2023.02.27</sup> [df35201](https://gitee.com/dotnetchina/Furion/commit/df35201622f4d908ab423baff27caef856a23527)
...
`ASP.NET Core` 和 `Furion` 框架都提供了标准化日志输出，如果对颜色，格式有要求，可使用下列代码进行自定义。
...
在 `Furion 4.8.5.4` 版本之前，如果在 `DatabaseLoggingWriter` 中注入 `ILogger<>` 对象将会导致死循环，原因是 `IDatabaseLoggingWriter` 本身就是日志输出的最终介质，如果这里还输出日志，将导致递归初始化日志实例。
...
可查看相关 `Issue`：[https://gitee.com/dotnetchina/Furion/issues/I7KJ5H](https://gitee.com/dotnetchina/Furion/issues/I7KJ5H)
...
想了解更多 `日志` 知识可查阅 [ASP.NET Core - 日志](https://docs.microsoft.com/zh-cn/aspnet/core/fundamentals/logging/?view=aspnetcore-5.0) 章节。
