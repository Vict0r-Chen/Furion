...
public class ComponentMetadataTests
{
    ...
    public void New_Invalid_Parameters()
    {
        ...
        {
            var componentMetadata = new ComponentMetadata(null!, null!, null!);
        });

        Assert.Throws<ArgumentException>(() =>
        {
            var componentMetadata = new ComponentMetadata(string.Empty, null!, null!);
        });

        Assert.Throws<ArgumentException>(() =>
        {
            var componentMetadata = new ComponentMetadata("", null!, null!);
        });
    }
    ...
    public void New_ReturnOK()
    {
        ...
        Assert.Equal("Furion 框架核心模块。", componentMetadata.Description);
        Assert.Equal("https://www.nuget.org/packages/Furion.Core/" + version?.ToString(), componentMetadata.NuGetPage);
        Assert.Equal("https://furion.net/docs/Furion.Core", componentMetadata.DocumentationPage);
    }
}