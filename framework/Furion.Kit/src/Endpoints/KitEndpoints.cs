...
internal static class KitEndpoints
{
    ...
    /// <param name="jsonOptions"><see cref="IOptions{TOptions}"/></param>
    /// <returns><see cref="IResult"/></returns>
    internal static IResult OpenApiHandler(HttpContext httpContext, IApiDescriptionGroupCollectionProvider provider, IOptions<JsonOptions> jsonOptions)
    {
        // 初始化 API 描述器解析器
        var apiDescriptionParser = new ApiDescriptionParser(provider, jsonOptions);
