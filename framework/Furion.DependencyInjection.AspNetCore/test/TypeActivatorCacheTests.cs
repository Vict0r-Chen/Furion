...
public class TypeActivatorCacheTests
{
    ...
    [Fact]
    public void CreateInstance_Null_Throw()
    {
        var typeActivatorCache = new TypeActivatorCache();

        Assert.Throws<ArgumentNullException>(() =>
        {
            typeActivatorCache.CreateInstance<object>(null!, null!);
        });

        Assert.Throws<ArgumentNullException>(() =>
        {
            typeActivatorCache.CreateInstance<object>(null!, typeof(ControllerA));
        });
        ...
        Assert.Throws<ArgumentNullException>(() =>
        {
            ...
            typeActivatorCache.CreateInstance<object>(serviceProvider, null!);
        });
    }
    ...
    public void CreateInstance_ReturnOK()
    {
        ...
        var instance = typeActivatorCache.CreateInstance<object>(serviceProvider, typeof(ControllerA));
        Assert.NotNull(instance);

        var controllerA = (ControllerA)instance;
        Assert.NotNull(controllerA);

        Assert.Single(typeActivatorCache._typeActivatorCache);

        var instance2 = typeActivatorCache.CreateInstance<object>(serviceProvider, typeof(ControllerA));
        Assert.NotNull(instance);
