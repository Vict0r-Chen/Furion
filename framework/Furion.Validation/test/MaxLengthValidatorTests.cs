...
public class MaxLengthValidatorTests
{
    ...
    {
        var validator = new MaxLengthValidator(1);
        ...
        Assert.False(validator.IsValid(new HashSet<string> { "furion", "fur" }));
    }
    ...
    public void Default_ErrorMessage()
    {
        ...
        Assert.Equal("The field must be a string or array type with a maximum length of 5.", failure.ErrorMessage);

        var failure2 = validator.GetValidationResult("furion", new List<string> { "Value" });
        Assert.NotNull(failure2);
        Assert.Equal("The field Value must be a string or array type with a maximum length of 5.", failure2.ErrorMessage);
    }
