...
public class CurrencyAmountValidatorTests
{
    [Theory]
    ...
    [InlineData(13000, true)]
    [InlineData(13.0, true)]
    [InlineData(13.56, true)]
    [InlineData(13.2, true)]
    [InlineData(14.03, true)]
    ...
    {
        var validator = new CurrencyAmountValidator();
    ...
    [InlineData("0.0", true)]
    [InlineData("-10", true)]
    public void AllowNegative_IsValid(object? value, bool result)
    {
        var validator = new CurrencyAmountValidator
        {
            AllowNegative = true
    ...
    public void Default_ErrorMessage()
    {
        ...
        var failure = validator.GetValidationResult(123.456m);
    ...
    public void Custom_ErrorMessage()
    {
        var validator = new CurrencyAmountValidator
        {
            ErrorMessage = "不是一个有效的货币金额格式"
        };

        var failure = validator.GetValidationResult(123.456m);
        Assert.NotNull(failure);

        Assert.Equal("不是一个有效的货币金额格式", failure.ErrorMessage);
    }
}