...
public class StandardPlateNumberValidatorTests
{
    [Theory]
    [InlineData(null, true)]
    [InlineData("京A00599", true)]
    [InlineData("黑D23908", true)]
    [InlineData("京AD92035", false)]
    [InlineData("甘G23459F", false)]
    [InlineData("京AA92035", false)]
    [InlineData("京A12345D", false)]
    ...
    {
        var validator = new StandardPlateNumberValidator();
    ...
    public void Default_ErrorMessage()
    {
        ...
        var failure = validator.GetValidationResult("京AD92035");
    ...
    public void Custom_ErrorMessage()
    {
        var validator = new StandardPlateNumberValidator
        {
            ErrorMessage = "不是一个有效的标准车牌号格式"
        };

        var failure = validator.GetValidationResult("京AD92035");
        Assert.NotNull(failure);

        Assert.Equal("不是一个有效的标准车牌号格式", failure.ErrorMessage);
    }
}