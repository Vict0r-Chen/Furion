// 麻省理工学院许可证
//
// 版权所有 © 2020-2023 百小僧，百签科技（广东）有限公司
//
// 特此免费授予获得本软件及其相关文档文件（以下简称“软件”）副本的任何人以处理本软件的权利，
// 包括但不限于使用、复制、修改、合并、发布、分发、再许可、销售软件的副本，
// 以及允许拥有软件副本的个人进行上述行为，但须遵守以下条件：
//
// 在所有副本或重要部分的软件中必须包括上述版权声明和本许可声明。
//
// 软件按“原样”提供，不提供任何形式的明示或暗示的保证，包括但不限于对适销性、适用性和非侵权的保证。
// 在任何情况下，作者或版权持有人均不对任何索赔、损害或其他责任负责，
// 无论是因合同、侵权或其他方式引起的，与软件或其使用或其他交易有关。

namespace Furion.Validation.Tests;

public class ValueAnnotationValidatorTests
{
    [Fact]
    public void New_Invalid_Parameters()
    {
        Assert.Throws<ArgumentNullException>(() =>
        {
            var validator = new ValueAnnotationValidator(null!);
        });

        Assert.Throws<ArgumentNullException>(() =>
        {
            var validator = new ValueAnnotationValidator((IList<ValidationAttribute>)null!);
        });

        var exception = Assert.Throws<ArgumentException>(() =>
        {
            var validator = new ValueAnnotationValidator(new RequiredAttribute(), null!, new MinLengthAttribute(3));
        });
        Assert.Equal("The validation attribute collection contains a null value. (Parameter 'validationAttributes')", exception.Message);
    }

    [Fact]
    public void New_Default()
    {
        var validator = new ValueAnnotationValidator();

        Assert.NotNull(validator);
        Assert.NotNull(validator.Attributes);
        Assert.Empty(validator.Attributes);
        Assert.Null(validator.ErrorMessage);
        Assert.NotNull(validator._errorMessageResourceAccessor);
        Assert.Equal("The field {0} is invalid.", validator._errorMessageResourceAccessor());
    }

    [Fact]
    public void IsValid_Invalid_Parameters()
    {
        Assert.Throws<ArgumentNullException>(() =>
        {
            var validator = new ValueAnnotationValidator();
            validator.IsValid(null!);
        });

        var exception = Assert.Throws<ArgumentException>(() =>
        {
            var validator = new ValueAnnotationValidator();
            validator.Attributes.Add(null!);
            validator.IsValid(null);
        });
        Assert.Equal("The validation attribute collection contains a null value. (Parameter 'validationAttributes')", exception.Message);
    }

    [Fact]
    public void IsValid_EmptyAttributes_ReturnOK()
    {
        var validator = new ValueAnnotationValidator();
        Assert.True(validator.IsValid(string.Empty));

        validator.Attributes.Add(new RequiredAttribute());
        Assert.False(validator.IsValid(null));
    }

    [Fact]
    public void IsValid_ReturnOK()
    {
        var validator = new ValueAnnotationValidator(new RequiredAttribute(), new MinLengthAttribute(3));
        Assert.False(validator.IsValid(null));
        Assert.False(validator.IsValid(string.Empty));
        Assert.False(validator.IsValid("赢"));
        Assert.True(validator.IsValid("百小僧"));
    }

    [Fact]
    public void GetValidationResults_Invalid_Parameters()
    {
        Assert.Throws<ArgumentNullException>(() =>
        {
            var validator = new ValueAnnotationValidator();
            var validationResults = validator.GetValidationResults(null!, "data");
        });

        var exception = Assert.Throws<ArgumentException>(() =>
        {
            var validator = new ValueAnnotationValidator();
            validator.Attributes.Add(null!);
            var validationResults = validator.GetValidationResults(null, "data");
        });
        Assert.Equal("The validation attribute collection contains a null value. (Parameter 'validationAttributes')", exception.Message);
    }

    [Fact]
    public void GetValidationResults_EmptyAttributes_ReturnOK()
    {
        var validator = new ValueAnnotationValidator();
        Assert.Null(validator.GetValidationResults(string.Empty, "data"));
        Assert.Null(validator.GetValidationResults("百小僧", "data"));
    }

    [Fact]
    public void GetValidationResults_ReturnOK()
    {
        var validator = new ValueAnnotationValidator(new RequiredAttribute(), new MinLengthAttribute(3));

        var validationResultsOfSucceed = validator.GetValidationResults("百小僧", "data");
        Assert.Null(validationResultsOfSucceed);

        var validationResultsOfFailure = validator.GetValidationResults("赢", "data");
        Assert.NotNull(validationResultsOfFailure);
        Assert.Single(validationResultsOfFailure);
        Assert.Equal("data", validationResultsOfFailure.First().MemberNames.First());
        Assert.Equal("The field data must be a string or array type with a minimum length of '3'.", validationResultsOfFailure.First().ErrorMessage);
    }

    [Fact]
    public void GetValidationResults_WithErrorMessage_ReturnOK()
    {
        var validator = new ValueAnnotationValidator(new RequiredAttribute(), new MinLengthAttribute(3))
        {
            ErrorMessage = "数据 {0} 验证失败"
        };

        var validationResultsOfFailure = validator.GetValidationResults("赢", "data");
        Assert.NotNull(validationResultsOfFailure);
        Assert.Equal("数据 data 验证失败", validationResultsOfFailure.First().ErrorMessage);
    }

    [Fact]
    public void FormatErrorMessage_ReturnOK()
    {
        var validator = new ValueAnnotationValidator();

        Assert.Equal("The field data is invalid.", validator.FormatErrorMessage("data"));
    }

    [Fact]
    public void Validate_Invalid_Parameters()
    {
        Assert.Throws<ArgumentNullException>(() =>
        {
            var validator = new ValueAnnotationValidator();
            validator.Validate(null!, "data");
        });

        var exception = Assert.Throws<ArgumentException>(() =>
        {
            var validator = new ValueAnnotationValidator();
            validator.Attributes.Add(null!);
            validator.Validate(null, "data");
        });
        Assert.Equal("The validation attribute collection contains a null value. (Parameter 'validationAttributes')", exception.Message);
    }

    [Fact]
    public void Validate_ReturnOK()
    {
        var validator = new ValueAnnotationValidator(new RequiredAttribute(), new MinLengthAttribute(3));

        validator.Validate("百小僧", "data");
    }

    [Fact]
    public void Validate_Failure()
    {
        var validator = new ValueAnnotationValidator(new RequiredAttribute(), new MinLengthAttribute(3));

        var exception = Assert.Throws<AggregateValidationException>(() =>
        {
            validator.Validate("赢家", "data");
        });

        Assert.Single(exception.InnerExceptions);
        Assert.Equal("The field data must be a string or array type with a minimum length of '3'.", exception.InnerExceptions.First().Message);
    }

    [Theory]
    [InlineData(typeof(ValueAnnotationValidator), true)]
    [InlineData(typeof(ValidationAttribute), false)]
    public void IsSameAs_ReturnOK(Type type, bool result)
    {
        var validator = new ValueAnnotationValidator();

        Assert.Equal(result, validator.IsSameAs(type));
    }

    [Fact]
    public void EnsureLegalData_Invalid_Parameters()
    {
        Assert.Throws<ArgumentNullException>(() =>
        {
            ValueAnnotationValidator.EnsureLegalData(null!);
        });

        Assert.Throws<ArgumentNullException>(() =>
        {
            ValueAnnotationValidator.EnsureLegalData(null!);
        });

        var exception = Assert.Throws<ArgumentException>(() =>
        {
            ValueAnnotationValidator.EnsureLegalData(new List<ValidationAttribute> { new RequiredAttribute(), null!, new MinLengthAttribute(3) });
        });
        Assert.Equal("The validation attribute collection contains a null value. (Parameter 'validationAttributes')", exception.Message);
    }

    [Fact]
    public void TryValidate_Invalid_Parameters()
    {
        Assert.Throws<ArgumentNullException>(() =>
        {
            var validator = new ValueAnnotationValidator(new MinLengthAttribute(3));
            validator.TryValidate(null!, null!, out _);
        });
    }

    [Fact]
    public void TryValidate_ReturnOK()
    {
        var validator = new ValueAnnotationValidator(new RequiredAttribute(), new MinLengthAttribute(3));

        var result = validator.TryValidate("百小僧", "data", out var validationResults);
        Assert.True(result);
        Assert.NotNull(validationResults);
        Assert.Empty(validationResults);

        var result2 = validator.TryValidate("赢", "data", out var validationResults2);
        Assert.NotNull(validationResults2);
        Assert.Single(validationResults2);
        Assert.Equal("data", validationResults2.First().MemberNames.First());
        Assert.Equal("The field data must be a string or array type with a minimum length of '3'.", validationResults2.First().ErrorMessage);
    }
}